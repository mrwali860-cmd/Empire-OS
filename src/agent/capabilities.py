...
class CapabilityContract:
    ...
    def execute(self, task: Task) -> CapabilityResult:
        ...
        if isinstance(raw, CapabilityResult):
            ...
            result = CapabilityResult(ok=bool(raw.get("ok", False)), capability=str(raw.get("capability", self.name)), data=raw.get("data") if isinstance(raw.get("data"), dict) else {}, error=raw.get("error"))
        else:
...
class EmpireCapabilityExecutor:
    ...
    def _verify_project_inspection(result: CapabilityResult) -> bool:
        data = result.data or {}
        return result.ok and result.error is None and isinstance(data.get("project_root"), str) and bool(data["project_root"]) and isinstance(data.get("files"), int) and not isinstance(data.get("files"), bool) and data["files"] >= 0 and isinstance(data.get("directories"), int) and not isinstance(data.get("directories"), bool) and data["directories"] >= 0
    ...
    def _verify_test_runner(result: CapabilityResult) -> bool:
        data = result.data or {}
        return result.ok and result.error is None and isinstance(data.get("return_code"), int) and not isinstance(data.get("return_code"), bool) and data["return_code"] == 0 and isinstance(data.get("stdout"), str) and len(data["stdout"]) <= EmpireCapabilityExecutor.TEST_OUTPUT_MAX_CHARS and isinstance(data.get("stderr"), str) and len(data["stderr"]) <= EmpireCapabilityExecutor.TEST_OUTPUT_MAX_CHARS
    ...
    def _verify_decision_engine(result: CapabilityResult) -> bool:
        ...
        if not isinstance(data.get("context"), dict) or not isinstance(data.get("recommended"), dict) or not isinstance(data.get("alternatives"), list) or len(data["alternatives"]) > DecisionEngineCapability.MAX_OPTIONS:
            return False
        return EmpireCapabilityExecutor._valid_decision_result(data["recommended"]) and all(EmpireCapabilityExecutor._valid_decision_result(item) for item in data["alternatives"])
    ...
    def run_tests(self, task: Task) -> CapabilityResult:
        ...
        return CapabilityResult(ok=completed.returncode == 0, capability="test_runner", data={"return_code": completed.returncode, "stdout": completed.stdout[-self.TEST_OUTPUT_MAX_CHARS:], "stderr": completed.stderr[-self.TEST_OUTPUT_MAX_CHARS:]}, error=f"Test suite failed with exit code {completed.returncode}." if completed.returncode != 0 else None)