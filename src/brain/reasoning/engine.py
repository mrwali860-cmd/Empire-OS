...
class ReasoningResult:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {"goal": self.goal, "assumptions": list(self.assumptions), "constraints": list(self.constraints), "next_actions": list(self.next_actions), "confidence": self.confidence}

    def summary(self) -> str:
        actions = "\n".join(f"{index}. {action}" for index, action in enumerate(self.next_actions, start=1))
...
class ReasoningEngine:
    ...
    def _project_search_query(text: str) -> str:
        ...
        prefixes = ("search project for ", "search code for ", "search repository for ", "find in project ", "find in code ", "find code ")
    ...
    def reason(self, user_input: str, intent: str, context: dict[str, Any], thinking_result: str) -> ReasoningResult:
        ...
        if context.get("budget") != "UNKNOWN": constraints.append(f"Budget: {context['budget']}")
        ...
        if intent == "GIT_STATUS":
            ...
            actions = (f"Search project source files for: {self._project_search_query(text)}",)
