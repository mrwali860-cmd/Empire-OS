        ...
        if isinstance(tasks, list):
            for index, task in enumerate(tasks, start=1):
                if isinstance(task, dict):
                    lines.extend([f"{index}. [{task.get('status', 'PENDING')}] {task.get('title', 'Untitled task')}", f"   Action: {task.get('action', 'MANUAL_REVIEW')}", f"   Permission: {'REQUIRED' if task.get('requires_permission') else 'NOT REQUIRED'}", f"   Verification: {task.get('verification', 'Verify outcome before completion.')}"])
                else: lines.append(f"{index}. {task}")
        else: lines.append(str(tasks))
        if orchestration_result is not None:
            lines.extend(["", f"Execution Status: {str(orchestration_result.get('status', 'unknown')).upper()}", f"Completed Tasks: {orchestration_result.get('completed_tasks', 0)}"])
            ...
            if capability_results:
                ...
                for index, capability_result in enumerate(capability_results, start=1):
                    ...
                    if capability == "file_read":
                        ...
                        lines.append(f"      Path: {data.get('path', 'unknown')}")
                        lines.append(f"      Bytes Written: {data.get('bytes_written', 'unknown')}")
                        ...
                        lines.append(f"      Query: {data.get('query', 'unknown')}")
                        lines.append(f"      Matches: {len(data.get('matches', []))}")
                        lines.append(f"      Truncated: {data.get('truncated', 'unknown')}")
                        for match in data.get('matches', [])[:10]: lines.append(f"      {match.get('file', 'unknown')}:{match.get('line', '?')} — {match.get('text', '')}")
                    ...
                    if capability_result.get("error"): lines.append(f"      Error: {capability_result['error']}")
            audit = orchestration_result.get("audit", [])
            if audit:
                lines.append(f"Audit Records: {len(audit)}")
                for index, record in enumerate(audit, start=1):
                    lines.append(f"   Audit {index}: {record.get('task_id', 'unknown')} → {str(record.get('status', 'unknown')).upper()} / {'VERIFIED' if record.get('verified') else 'UNVERIFIED'}")
                    if record.get("error"): lines.append(f"      Error: {record['error']}")
            if orchestration_result.get("failed_task_id"): lines.append(f"Failed Task: {orchestration_result['failed_task_id']}")
            if orchestration_result.get("error"): lines.append(f"Execution Error: {orchestration_result['error']}")
        lines.extend(["", "Status: READY FOR EXECUTION" if orchestration_result is None else "Status: EXECUTION PROCESSED", f"Verification: {'REQUIRED' if plan.get('verification_required', True) else 'OPTIONAL'}", "===============================", ""])
        return "\n".join(lines)
